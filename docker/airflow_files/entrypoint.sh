...
function verify_db_connection {
    ...
    if [[ ${DB_URL} != sqlite* ]]; then
        ...
        echo DB_HOST="${DETECTED_DB_HOST}"
        echo DB_PORT="${DETECTED_DB_PORT}"

        while true
        do
            set +e
            LAST_CHECK_RESULT=$(nc -zvv "${DETECTED_DB_HOST}" "${DETECTED_DB_PORT}" >/dev/null 2>&1)
            RES=$?
            set -e
            if [[ ${RES} == 0 ]]; then
                echo
            ...
            if [[ ${DB_CHECK_MAX_COUNT} == 0 ]]; then
                echo
                echo "ERROR! Maximum number of retries (${DB_CHECK_MAX_COUNT}) reached while checking ${DETECTED_DB_BACKEND} db. Exiting"
                echo
                break
            else
        ...
        if [[ ${RES} != 0 ]]; then
            echo "        ERROR: ${DETECTED_DB_BACKEND} db could not be reached!"
            echo
